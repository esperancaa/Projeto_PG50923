...
def evaluate(predictions):
    ...
    for batch in predictions:
        for i in range(batch[0].shape[0]):
            ...
            if source == 1:
                ...
                kiba_pred.append(pred)
                kiba_target.append(target)
    ...
    kiba_pred_label = np.where(np.array(kiba_pred) >= 0.5, 1, 0)
    ...
    try:
        kiba_auroc = roc_auc_score(kiba_target, kiba_pred).round(4)
        kiba_auprc = average_precision_score(kiba_target, kiba_pred).round(4)
        kiba_sen, kiba_spec = compute_sen_spec(kiba_target, kiba_pred_label)
    except:
        kiba_auroc, kiba_auprc, kiba_sen, kiba_spec = -1, -1, -1, -1
