...
def Protein_embedding_process(dataset, fold, dir_output, id_train, id_test):
    ...
    for no, id in enumerate(id_train):
        print('/'.join(map(str, [no + 1, N])))
        protein_embedding_train = np.load('data/' + dataset + '/processed' + '/ESM_embedding_pocket/' + str(id) + '.npy', allow_pickle=True)
    ...
    for no, id in enumerate(id_test):
        print('/'.join(map(str, [no + 1, N])))
        protein_embedding_test = np.load('data/' + dataset + '/processed' + '/ESM_embedding_pocket/' + str(id) + '.npy', allow_pickle=True)
